...
class SCCAdapter extends utils.Adapter {
    ...
    async onReady() {
        ...
        try {
            ...
            const rules = this._normalizeRules(this._parseArray(config.rules));
            ...
            try {
                const healthResult = await runHealthCheck(this, this.config);
                await this.setStateAsync(this.namespace + '.info.health', JSON.stringify(healthResult), true);
                if (!healthResult.ok && healthResult.issues.length > 0) {
                    this.log.warn('Health-Check: ' + healthResult.issues.join('; '));
                }
            } catch (e) {
                this.log.warn('Health-Check: ' + (e && e.message ? e.message : String(e)));
            }
            ...
            this.log.info('SCC adapter started. Surplus threshold: ' + (this.config.surplusThresholdW || 50) + ' W. Loglevel "debug" für Details.');
            const port = Math.max(0, parseInt(this.config.standalonePort, 10) || 0);
            if (port > 0) {
                try {
                    await this._startStandaloneServer(port);
                    this.log.info('SCC: Flow-Einzelseite unter http://<Host>:' + port + '/flow.html');
                } catch (e) {
                    this.log.warn('SCC: Standalone-Server konnte nicht starten: ' + (e && e.message ? e.message : String(e)));
                }
            }
        } catch (err) {
    ...
    onMessage(obj) {
        if (!obj || obj.command !== 'getFlowData') return;
        ...
        const collect = async () => {
            ...
            const data = { surplus: {}, consumption: {}, batteries: {}, grid: {}, generation: {} };
            try {
    ...
    async getFlowDataAsync() {
        ...
        const data = { surplus: {}, consumption: {}, batteries: {}, grid: {}, generation: {}, sourcesList: [], rulesList: [] };
        try {
            const powerW = await this.getStateAsync(prefix + 'surplus.powerW');
            const availableW = await this.getStateAsync(prefix + 'surplus.availableForDevicesW');
            const feedInW = await this.getStateAsync(prefix + 'surplus.feedInW');
    ...
    async readInitialStates() {
        ...
        for (const b of this.config.batteries || []) {
            ...
            if (socId) {
                try {
                    const st = await this.getStateAsync(socId);
            ...
            if (chargeId) {
                try {
                    const st = await this.getStateAsync(chargeId);
            ...
            if (dischargeId) {
                try {
                    const st = await this.getStateAsync(dischargeId);
    ...
    async tick() {
        ...
        try {
            ...
            const availableW = availableForDevicesW(surplusW, reservedW, totalDischargeW, this.config.surplusPriority);
            ...
            if (this.config.forecastEnabled && this.config.forecastSourceId) {
                try {
                    const forecastState = await this.getStateAsync(this.config.forecastSourceId);
                    if (forecastState && forecastState.val != null) {
                        const v = forecastState.val;
                        if (typeof v === 'number' && !isNaN(v)) {
                            let w = Math.max(0, v);
                            const obj = await this.getObjectAsync(this.config.forecastSourceId);
