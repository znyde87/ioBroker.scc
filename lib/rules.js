...
RuleEngine.prototype.evaluate = async function (availableForDevicesW) {
    ...
    for (const r of sorted) {
        ...
        const wantOn = avail >= thresholdOn;
        ...
        if (wantOn && !state.currentOn) {
            if (delayOnSec > 0) {
                ...
                if (delayElapsed >= delayOnSec) {
                    if (minOffSec > 0 && state.offSince && (now - state.offSince) / 1000 < minOffSec) {
                        ...
                        else if (debugRules) this.adapter.log.debug(`[Regel ${ruleId}] Simulation: würde EIN schalten (avail=${avail}W >= ${thresholdOn}W)`);
                    }
            ...
            this.adapter.log.debug(`[Regel ${ruleId}] Kein EIN: avail ${avail}W < Schwellwert ${thresholdOn}W`);
        } else if (!wantOn) {
            state.delayOnAt = null;
        }
