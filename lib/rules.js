...
RuleEngine.prototype._evaluatePid = async function (r, ruleId, availableForDevicesW, now, simulationMode, debugRules) {
    ...
    if (inputSource === 'state' && inputStateId) {
        try {
            ...
            if (debugRules) this.adapter.log.debug(`[Regel ${ruleId}] PID Eingang lesen: ${e.message}`);
        }
        ...
        try {
            ...
            if (debugRules) this.adapter.log.debug(`[Regel ${ruleId}] PID Temperatur lesen: ${e.message}`);
            this.ruleState[ruleId] = state;
            return;
        }
    }
    ...
    if (tempLimitId && tempLimitMax != null) {
        try {
            ...
            if (tempVal != null && !isNaN(tempVal) && tempVal >= tempLimitMax) {
                state.lastOutput = 0;
                state.lastI = 0;
                state.pidDebug = Object.assign({}, pidDebug, { output: 0, reason: 'Übertemperatur ≥ ' + tempLimitMax + ' °C' });
                if (!simulationMode) await this.setTargetStateValue(outputStateId, 0);
                if (debugRules) this.adapter.log.debug(`[Regel ${ruleId}] Übertemperatur: ${tempVal} °C >= ${tempLimitMax} °C → Ausgang 0 %`);
                this.ruleState[ruleId] = state;
                return;
            }
        } catch (e) {
            if (debugRules) this.adapter.log.debug(`[Regel ${ruleId}] Temperatur-Limit lesen: ${e.message}`);
        }
    }
